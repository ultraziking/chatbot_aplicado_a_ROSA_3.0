...
def stop_ros():
    if messagebox.askokcancel("Stop ROS", "Are you sure? This will close every node"):
        global flag_master
        flag_master = False
        print("Killing nodes...")
...
def close():
    if messagebox.askokcancel ("Close RoSA","This will quit RoSA_interface\nAre you sure?"):
        stop_ros()
        root.destroy()


def shutdown():
    if messagebox.askokcancel("Shut down RoSA", "Are you sure?"):
        if messagebox.askokcancel( "Are you really sure? Remember to save everything before shut down"):
            root.destroy()
            os.system("poweroff")


def restart():
    if messagebox.askokcancel("Restart RoSA", "Are you sure?"):
        if messagebox.askokcancel( "Are you really sure? Remember to save everything before restart"):
            root.destroy()
            os.system("reboot")
...
def information():
    messagebox.showinfo("About RoSA", "RoSA is a social assistant robot designed to be a guide in hospitals, events, etc.")

def help():
    messagebox.showinfo("Help", "It is very simple to use RoSA\n1.Pulse the botton Start ROS\n2.Pulse Start RoSA\n3.Choose the type of Navigation\nFinaly if you want to visualize the robot model and the data sensors pulse Start Rviz")

def start_RoSA():

    global pid_model
    global flag_rosa
    if messagebox.askokcancel("Start RoSA", "Remember to Start ROS before Start RoSA"):	 
        flag_rosa = True   
        print ("Model started under process ")
...
def chat(x):
    ...
    global salida
    global input_voz
    roboto = ["sabes qué es un robot antropomorfico?", "sabes qué es un robot pintor?", "sabes qué es un robot con patas?"]
    robota = ["do you know what is a anthropomorphic robot?", "do you know what is a painter robot?", "do you know what is a legged robot?"]
    while x==1: 
        if flag_castellano == True:
                ...
                try:
                    with sr.Microphone() as source:   
                        ...
                        if inp =="adios" or inp =="adiós":
                            ...
                            tipo = random.choice(roboto)
                            salida.say(tipo)
                            salida.runAndWait()

                            with sr.Microphone() as sourca:   
                                input_voz.adjust_for_ambient_noise(sourca, duration=0.2)
                                aud = input_voz.listen(sourca)
                                tex = input_voz.recognize_google(aud, language = "es-ES")
                                i = format(tex)
                            
                                if tipo == "sabes qué es un robot antropomorfico?":
                                    if i == "no" or i == "no lo se" or i == "no sé" or i == "no lo sé":
                                        salida.say("un robot antropomórfico es aquel que tiene tanto movimiento rotacional como angular, puede tener tres o mas grados de libertad, en nuestro laboratorio tenemos un robot pintor de este tipo")
                                        salida.runAndWait()
                                    elif i == "si" or i == "sí" or i == "lo sé":
                                        salida.say("si quieres verlo puedes decir vamos al laboratorio y cambiamos al modo navegación")
                                        salida.runAndWait()
                                elif tipo == "sabes qué es un robot pintor?":
                                    if i == "no" or i == "no lo se" or i == "no sé" or i == "no lo sé":
                                        salida.say("un robot pintor es un robot antropomórfico capaz de dibujar en un cuadro, tenemos un robot pintor en el laboratorio de robótica, quieres verlo?")
                                        salida.runAndWait()
                                    elif i == "si" or i == "sí" or i == "lo sé":
                                        salida.say("si quieres verlo puedes decir vamos al laboratorio y cambiamos al modo navegación")
                                        salida.runAndWait()
                                elif tipo == "sabes qué es un robot con patas?":
                                    if i == "no" or i == "no lo se" or i == "no sé" or i == "no lo sé":
                                        salida.say("Los robots con patas son un tipo de robot móvil que usa extremidades mecánicas para el movimiento. Son más versátiles que los robots con ruedas y pueden atravesar muchos terrenos diferentes")
                                        salida.runAndWait()
                                    elif i == "si" or i == "sí" or i == "lo sé":
                                        salida.say("si quieres verlo puedes decir vamos al laboratorio y cambiamos al modo navegación")
                                        salida.runAndWait()
                    ...
                    with sr.Microphone() as source:   
                        ...
                        if inp =="bye" or inp =="good bye" or inp == "see you" or inp == "exit":
                            ...
                            with sr.Microphone() as sourca:   
                                input_voz.adjust_for_ambient_noise(sourca, duration=0.2)
                                aud = input_voz.listen(sourca)
                                tex = input_voz.recognize_google(aud, language = "en-GB")
                                i = format(tex)
                            
                                if tipo == "do you know what is a anthropomorphic robot?":
                                    if i == "no" or i == "I don't know":
                                        salida.say("un robot antropomórfico es aquel que tiene tanto movimiento rotacional como angular, puede tener tres o mas grados de libertad, en nuestro laboratorio tenemos un robot pintor de este tipo")
                                        salida.runAndWait()
                                    elif i == "yes" or i == "yes i do":
                                        salida.say("if you want to see it you can say let's go to the laboratory and I will switch to navigation mode")
                                        salida.runAndWait()
                                elif tipo == "do you know what is a painter robot?":
                                    if i == "no" or i == "I don't know":
                                        salida.say("a painting robot is an anthropomorphic robot capable of drawing on a canvas, we have a painting robot in the robotics laboratory, do you want to see it?")
                                        salida.runAndWait()
                                    elif i == "yes" or i == "yes i do":
                                        salida.say("if you want to see it you can say let's go to the laboratory and I will switch to navigation mode")
                                        salida.runAndWait()
                                elif tipo == "do you know what is a legged robot?":
                                    if i == "no" or i == "I don't know":
                                        salida.say("Legged robots are a type of mobile robot that uses mechanical limbs for movement. They are more versatile than wheeled robots and can traverse many different terrains")
                                        salida.runAndWait()
                                    elif i == "yes" or i == "yes i do":
                                        salida.say("if you want to see it you can say let's go to the laboratory and I will switch to navigation mode")
                                        salida.runAndWait()
...
def start_rviz():
    ...
    messagebox.showinfo("About Rviz", "Rviz often fails to start. If so, close rviz and try again")
    if flag_slam:
        flag_slam = True
        print ("Rviz started under process ")
    elif flag_nav:
        flag_nav = True
        print ("Navegacion started under process ")
    else:
        flag_rviz = True
        print ("Rviz started under process ")
...
def start_nav():
    ...
    if flag_gamepad:
       flag_gamepad = False
    flag_nav = True
    print ("Navigation started under process ")
...
def start_slam():
    ...
    if flag_gamepad:
      flag_gamepad = False
    print ("SLAM started under process ")
    print ("Teleop started under process ")
...
image = Image.open("F:/我的U盘/TFG/ROSA/TODOS LOS CODIGOS DE ARDUINO Y OTROS PARA ROSA 3.0/rosa-3.0-master/rosa-3.0-master/catkin_ws/src/rosa_interface/src/ROS.png")
...
right_bar2.rowconfigure(4, weight=1)
...
#ETSIDI
image = Image.open("F:/我的U盘/TFG/ROSA/TODOS LOS CODIGOS DE ARDUINO Y OTROS PARA ROSA 3.0/rosa-3.0-master/rosa-3.0-master/catkin_ws/src/rosa_interface/src/ETSIDI.bmp")
[imageSizeWidth, imageSizeHeight] = image.size
scale = 0.3
image = image.resize((int(imageSizeWidth*scale), int(imageSizeHeight*scale)), Image.ANTIALIAS)
photo = ImageTk.PhotoImage(image)
...
#UPM
image = Image.open("F:/我的U盘/TFG/ROSA/TODOS LOS CODIGOS DE ARDUINO Y OTROS PARA ROSA 3.0/rosa-3.0-master/rosa-3.0-master/catkin_ws/src/rosa_interface/src/UPM.jpeg")
[imageSizeWidth, imageSizeHeight] = image.size
scale = 0.5
image = image.resize((int(imageSizeWidth*scale), int(imageSizeHeight*scale)), Image.ANTIALIAS)
photo = ImageTk.PhotoImage(image)
...
#Aura
image = Image.open("F:/我的U盘/TFG/ROSA/TODOS LOS CODIGOS DE ARDUINO Y OTROS PARA ROSA 3.0/rosa-3.0-master/rosa-3.0-master/catkin_ws/src/rosa_interface/src/3009aura.jpg")
[imageSizeWidth, imageSizeHeight] = image.size
scale = 0.7
image = image.resize((int(imageSizeWidth*scale), int(imageSizeHeight*scale)), Image.ANTIALIAS)
photo = ImageTk.PhotoImage(image)
...
#Arduino
image = Image.open("F:/我的U盘/TFG/ROSA/TODOS LOS CODIGOS DE ARDUINO Y OTROS PARA ROSA 3.0/rosa-3.0-master/rosa-3.0-master/catkin_ws/src/rosa_interface/src/arduino-logosvg.png")
[imageSizeWidth, imageSizeHeight] = image.size
scale = 0.1
image = image.resize((int(imageSizeWidth*scale), int(imageSizeHeight*scale)), Image.ANTIALIAS)
photo = ImageTk.PhotoImage(image)
